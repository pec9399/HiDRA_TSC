        ...
        try:
            while not context.stop:
                ...
                if len(self.node.instances) > 0 or len(self.child_instances) > 0:
                    ...
                    if total_requests > 0:
                        assert (total_response / total_requests <= config.timeout)
                ...
                if total_requests > 0:
                    ...
                    #response_time = total_handled/total_requests
                    response_time = total_response/total_requests
                    #print(response_time)


                    t = np.percentile(total_requests_record, [99])
                    #print(t)
                    t = t[0]

                    context.logger.debug(
                        f"Node {self.node.id} received {total_requests} with {len(self.node.instances)} instances, QoS {response_time}")
                    if t > self.threshold*config.timeout:
                        context.logger.debug(f"Node {self.node.id} Scale")
